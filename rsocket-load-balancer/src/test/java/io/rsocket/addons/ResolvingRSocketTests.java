...
public class ResolvingRSocketTests {
  ...
  @Test
  public void shouldReleasePayloadIfCancelAndAcceptRacing() {
    for (int i = 0; i < 10000; i++) {
  ...
  public void shouldExpireValueExactlyOnce() {
    for (int i = 0; i < 1000; i++) {
      ...
      final ResolvingRSocket resolvingRSocket =
          cold.mono().as(source -> new ResolvingRSocket(source));

      StepVerifier.create(ResolvingRSocket.subscribeOn(Schedulers.elastic()))
          .expectSubscription()
          .expectNext("value")
          .expectComplete()
          .verify(Duration.ofSeconds(timeout));
      ...
      Assertions.assertThat(received).hasSize(1).containsOnly(Tuples.of("value", ResolvingRSocket));
      RaceTestUtils.race(ResolvingRSocket::invalidate, ResolvingRSocket::invalidate);
      ...
      Assertions.assertThat(received)
          .hasSize(2)
          .containsOnly(Tuples.of("value", ResolvingRSocket), Tuples.of("value", ResolvingRSocket));
