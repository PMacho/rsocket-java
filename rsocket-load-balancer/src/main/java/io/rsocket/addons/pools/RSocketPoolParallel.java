...
public abstract class RSocketPoolParallel<S extends RSocket>
    implements RSocketPool, PoolOperations {
  ...
  private Flux<List<S>> activeRSockets() {
    return Flux.<List<S>>create(sink -> rSocketListConsumer = sink::next).as(this::hotSource);
  }
  ...
  private Disposable availablePoolUpdater(
      Publisher<? extends Collection<? extends RSocket>> rSocketsPublisher) {
  ...
  private Function<Collection<? extends RSocket>, Mono<List<S>>> socketList() {
    return rSockets ->
        Flux.fromIterable(rSockets).map(PooledRSocket::new).map(this::rSocketMapper).collectList();
  }
