...
public class ResolvingRSocket implements RSocket {
    ...
    ResolvingRSocket(Mono<? extends RSocket> source) {
        this.source = source;
            ...
            if (Operators.validate(n)) {
                ...
                if (r > STATE_SUBSCRIBED) { // works only in case onSubscribe has not happened
                    ...
                    for (; ; ) { // normal CAS loop with overflow protection
                        if (r
                                == Long
                                .MAX_VALUE) { // if r == Long.MAX_VALUE then we dont care and we can loose this
                            // request just in case of racing
                            return;
                        }
                        u = Operators.addCap(r, n);
                        if (REQUESTED.compareAndSet(
                                this, r, u)) { // Means increment happened before onSubscribe
                            return;
                        } else { // Means increment happened after onSubscribe
                            r = this.requested; // update new state to see what exactly happened (onSubscribe |
                            // cancel | requestN)
                ...
                this.s.request(
                        n); // if onSubscribe -> subscription exists (and we sure of that because volatile read
    ...
    static final class FlatMapManyInner<T> extends Flux<Payload>
        ...
        public void accept(RSocket rSocket, Throwable t) {
            if (rSocket == null) {
                ReferenceCountUtil.safeRelease(fluxOrPayload);
                Operators.error(this.actual, t);
                return;
            }
