...
public final class AtomicDouble extends Number {
   ...
   * expected value.
   *
   * <p><a href="package-summary.html#weakCompareAndSet">May fail spuriously and does not provide
   ...
   * current and given values, returning the updated value. The function should be side-effect-free,
   * since it may be re-applied when attempted updates fail due to contention among threads. The
   * function is applied with the current value as its first argument, and the given update as the
   * second argument.
   *
   ...
   * the previous value. The function should be side-effect-free, since it may be re-applied when
  ...
  /**
   * Atomically updates the current value with the results of applying the given function, returning
   * the updated value. The function should be side-effect-free, since it may be re-applied when
   * attempted updates fail due to contention among threads.
   *
   * @param updateFunction a side-effect-free function
   * @return the updated value
  ...
  private static long toLong(double delta) {
    return Double.doubleToLongBits(delta);
  }
}
